...
|------|-----------|-----------|
| Harmonize sampling rate | 100 Hz | Each session is linearly interpolated onto its native grid from metadata, then aligned to a common 100 Hz grid (all devices in this dataset report 100 Hz) |
...
| `walk_jump_energy_ratio` | Frequency (FFT) | Separates walking cadence from jump impacts |
| `gyro_mag_mean` | Time | Upright standing sway vs desk stillness |
| `accel_mag_std` | Time | Burstiness of jumping vs steadier walking |
...
| **Observations (X)** | 11-D Z-scored feature vectors per 1 s window |
...
**Training strategy:** Emission parameters are warm-started from labelled training windows. Baum–Welch refines the transition matrix on real single-activity recording sessions (each session is one steady activity in time order). Supervised emissions are restored after EM, and diagonal self-transitions are reinforced (minimum 0.95).
...
| Overall test accuracy | **83.3 %** (Viterbi per session) |
| Per-class sensitivity (unseen) | still 1.00 · standing 0.47 · walking 1.00 · jumping 0.88 |
...
*Figure 5 — True vs. Viterbi-decoded sequence on one held-out (`walking5`, end-to-end).*

### Unseen test evaluation

![Confusion matrix on unseen Jul 1 sessions](figures/06_confusion_matrix.png)

*Figure 6 — Confusion matrix on unseen test windows (Viterbi per session, 83.3 % overall accuracy).*
